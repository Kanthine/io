...
```
/** 计算字形数组的边界矩形，并返回这组字形的整体边界矩形框
 * @param font 指定的字体
 * @param orientation 字形的绘制方向
 * @param glyphs 一组字形
 * @param boundingRects 接收字形对应的边界框，可以传入 NULL
 * @param 字形和边界矩形缓冲区的容量
 */
...
/** 在上下文中渲染字体中指定的字形
 * @param font 用于渲染字形的字体。如果字体具有 size 或 matrix 属性，CGContext 将使用这些值设置
 * @param glyphs 待渲染的字形集合
 * @param positions 各个字形的坐标位置，两者的数量必须相等
 * @param count 要从字形数组中渲染的字形数量
 * @param context 上下文
 * 
 * @discussion 如果在 CTFont 中指定字体，该函数将修改上下文的字体、文本大小和文本矩阵，并且不会恢复
 *             给定的字形应该是 Unicode 文本布局操作的结果，如CTLine
 *             类似于 CTFontGetGlyphsForCharacters() 的 API 拿到的 CTFont 不执行任何Unicode文本布局
 */
...
/** 为指定的字形创建路径（字形轮廓），如果没有这样的字形轮廓，则返回 NULL
 * @param font 指定的字体
 * @param glyph 指定的字形
 * @param matrix 应用于路径的矩阵变换；为 NULL 则认为是单位矩阵
 *
 * @discussion 根据指定字体的字形轮廓创建路径。路径将按顺序反映字体点大小、矩阵和变换参数。
 */
