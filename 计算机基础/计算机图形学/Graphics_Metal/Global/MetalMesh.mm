    ...
    if(self)
    {
        ...
        if (textureLoader != NULL)
        {
            ...
            [mutableTextures addObject:
                [MetalSubmesh createMetalTextureFromMaterial: modelIOSubmesh.material
        ...
        NSAssert(TextureIndexBaseColor == 0, @"");
        NSAssert(TextureIndexSpecular == 1, @"");
        NSAssert(TextureIndexNormal == 2, @"");
...
{
    NSArray<MetalSubmesh *> *_submeshes;
}
                                      ...
                                      error:(NSError * __nullable * __nullable)error
{
    ...
    NSMutableArray<MetalSubmesh*>* mutableSubmeshes =
    ...
    for(NSUInteger index = 0; index < metalKitMesh.submeshes.count; index++)
    {
        ...
        MetalSubmesh *submesh =
        [[MetalSubmesh alloc] initWithModelIOSubmesh:modelIOMesh.submeshes[index]
                                               ...
                                               error:(NSError * __nullable * __nullable)error
{
    ...
    NSMutableArray<MetalMesh *> *newMeshes = [[NSMutableArray alloc] init];
    ...
    if ([object isKindOfClass:[MDLMesh class]])
    {
        ...
        MetalMesh *newMesh = [[MetalMesh alloc] initWithModelIOMesh:mesh
    ...
    {
        NSArray<MetalMesh*> *childMeshes;

        childMeshes = [MetalMesh newMeshesFromObject:child
                            modelIOVertexDescriptor:vertexDescriptor
                              metalKitTextureLoader:textureLoader
                                        metalDevice:device
                                              error:error];
                                              ...
                                              aabb:(MDLAxisAlignedBoundingBox&)aabb
{
    ...
    {
        NSArray<MetalMesh *> *assetMeshes;

        assetMeshes = [MetalMesh newMeshesFromObject:object
                            modelIOVertexDescriptor:vertexDescriptor
                              metalKitTextureLoader:textureLoader
                                        metalDevice:device
                                              error:error];
