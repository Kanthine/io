...
- (void)loadMetalWithMetalKitView:(nonnull MTKView *)mtkView {
    ...
    /**
     * 用于创建 multisampleColorTexture 的样本计数；默认为1。
     * 如果sampleCount大于1，一个多采样的颜色纹理将被创建，
     * currentDrawable的纹理将被设置为 currentRenderPassDescriptor 中的解析纹理，
     * 存储操作将被设置为 MTLStoreActionMultisampleResolve
     */
